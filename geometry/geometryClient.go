...
type Client interface {
	...
	GetArea(dimensions *pb.Dimensions) (*pb.Area, error)
}
...
func (c *client) Open(host, port string) error {
	...
	c.conn = conn
	c.agent = pb.NewGeometryClient(conn)
	c.host = host
	c.port = port
	return nil
}
...
func (c *client) GetArea(dimensions *pb.Dimensions) (*pb.Area, error) {
	ctx, cancel := context.WithTimeout(context.Background(), time.Second)
	defer cancel()
	return c.agent.GetArea(ctx, dimensions)
}
